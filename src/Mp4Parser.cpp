...
void Mp4ParserApp::ShowTracksTreeView()
{
    if (ImGui::BeginTabItem("Tracks"))
    {
        if (getMp4DataShare().dataAvailable)
        {
            ...
            // Baichuan Extra Data
            if (getMp4DataShare().mExtraInfo.size() > 0)
            {
                if (cur_track_select == getMp4DataShare().tracksInfo.size())
                    node_flags |= ImGuiTreeNodeFlags_Selected;
                else
                    node_flags &= (~ImGuiTreeNodeFlags_Selected);

                ImGui::TreeNodeEx((void *)(intptr_t)tree_node_id, node_flags, "extra info");
                tree_node_id++;
                if (isItemClicked())
                {
                    Z_INFO("clicked\n");
                    if (cur_track_select != getMp4DataShare().tracksInfo.size())
                        m_focus_changed = true;
                    cur_track_select = (int)getMp4DataShare().tracksInfo.size();
                }
            }
...
int Mp4ParserApp::updateData(int type, size_t trackIdx, size_t itemIdx)
{
    switch (type)
    {
        ...
        case 2:
        {
            if (itemIdx >= getMp4DataShare().mExtraInfo.size())
                return -1;

            auto &extraInfo = getMp4DataShare().mExtraInfo[itemIdx];
            if (mBinaryData.bufferSize < extraInfo.size)
            {
                mBinaryData.bufferSize = extraInfo.size;
                mBinaryData.buffer     = std::make_unique<uint8_t[]>(mBinaryData.bufferSize);
            }

            int ret = readDataFromFile(getMp4DataShare().getParser()->getFilePath(), mBinaryData.buffer.get(), extraInfo.offset,
                                       extraInfo.size);
            if (ret < 0)
                return ret;
            mBinaryData.type     = 2;
...
void Mp4ParserApp::updateSamplesTable()
{
    ...
    for (int i = 0; i < mSampleDataTables.size(); i++)
    {
...
void Mp4ParserApp::updateChunksTable()
{
    ...
    for (int i = 0; i < mChunkDataTables.size(); i++)
    {
...
// Baichuan Extra Data
void Mp4ParserApp::updateExtraTable()
{
    mExtraTable.setDataCallbacks([&]() { return getMp4DataShare().mExtraInfo.size(); },
                                 [&](size_t rowIdx, size_t colIdx) -> string
                                 {
                                     if (rowIdx >= getMp4DataShare().mExtraInfo.size())
                                         return "";
                                     auto &cur_item = getMp4DataShare().mExtraInfo[rowIdx];
                                     switch (colIdx)
                                     {
                                         case 0:
                                             return to_string(rowIdx + 1);
                                         case 1:
                                             if (getAppConfigure().needShowInHex)
                                                 return Log::format("{#x}", cur_item.offset);
                                             else
                                                 return to_string(cur_item.offset);
                                         case 2:
                                             if (getAppConfigure().needShowInHex)
                                                 return Log::format("{#x}", cur_item.size);
                                             else
                                                 return to_string(cur_item.size);
                                         default:
                                             return "";
                                     }
                                 },
                                 [&](size_t rowIdx, size_t colIdx) -> bool
                                 {
                                     if (rowIdx >= getMp4DataShare().mExtraInfo.size())
                                         return false;
                                     if (colIdx == 0)
                                 ...
                                 [&](size_t rowIdx, size_t colIdx) -> void
                                 {
                                     UNUSED(colIdx);
                                     ...
                                     Z_INFO("show Extra {}, offset {}, size {}\n", rowIdx, cur_item.offset, cur_item.size);

                                     if (updateData(2, 0, rowIdx) != 0)
                                         return;

                                     mDataViewer.setUserData(&cur_item);
                                     mDataViewer.setDataCallbacks(
                                         [](void *userData) -> ImS64
                                         {
                                             auto pExtra = static_cast<Mp4ParseData::ExtraInfo *>(userData);
                                             return pExtra->size;
                                         },
                                         [this](ImS64 offset, void *userData) -> uint8_t
                                         {
                                             auto pExtra = static_cast<Mp4ParseData::ExtraInfo *>(userData);
                                             if (offset < 0 || offset >= (int64_t)pExtra->size)
                                                 return 0;
                                         ...
                                         {
                                             auto pExtra = static_cast<Mp4ParseData::ExtraInfo *>(userData);
                                             saveCurrentData(fileName, pExtra->size);
                                         });
                                     mDataViewer.open();
                                 });
}
...
Mp4ParserApp::Mp4ParserApp() : mInfoWindow("Information")
{
    ...
    // Baichuan Extra Data
    mExtraTable.setTableFlag(TABLE_FLAGS);
    mExtraTable.addColumn("Index").addColumn("Offset").addColumn("Size");
    // End Baichuan Extra Data
}
...
void Mp4ParserApp::ShowInfoView()
{
    ...
    else if (FOCUS_ON_TRACKS == m_focus_on)
    {
        if (cur_track_select >= 0)
        {
            ...
            // Baichuan Extra Data
            else if (cur_track_select == getMp4DataShare().tracksInfo.size())
            {
                mExtraTable.show();
            }
            // End Baichuan Extra Data
...
void Mp4ParserApp::updateBoxDataTables(uint64_t boxIdx, const string &key, const Mp4BoxData &value)
{
    ...
    else if (MP4_BOX_DATA_TYPE_TABLE == value.getDataType())
    {
        ...
        for (auto header_idx = 0; header_idx < value.tableGetColumnCount(); header_idx++)
        {
